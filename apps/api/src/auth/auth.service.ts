...
export class AuthService {
  ...
  private async buildAuthenticatedUser(userRecord: typeof users.$inferSelect): Promise<AuthenticatedUser> {
  ...
  private async persistRefreshToken(userId: number, secret: string): Promise<{ refreshToken: string; refreshExpiresIn: number }> {
    const refreshExpiresIn = this.refreshTokenTtlSeconds
    const now = Date.now()
    const sessionId = randomUUID()
    const refreshToken = `${sessionId}.${secret}`

    await db.delete(refreshTokens).where(eq(refreshTokens.userId, userId)).run()
  ...
  private async issueSession(user: AuthenticatedUser): Promise<LoginResponse> {
    ...
    const { refreshToken, refreshExpiresIn } = await this.persistRefreshToken(user.id, secret)
